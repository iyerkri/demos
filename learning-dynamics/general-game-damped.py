...
for i in range(1,num_iter):
    ...
    payoffs[0,0] = strategy[i-1,1]*payoff_matrix[0,0] + (1-strategy[i-1,1])*payoff_matrix[0,1]
    payoffs[1,0] = strategy[i-1,1]*payoff_matrix[1,0] + (1-strategy[i-1,1])*payoff_matrix[1,1]

    # calculate payoff of player 2 assuming the other player plays according to empirical dist.
    payoffs[0,1] = strategy[i-1,0]*payoff_matrix[2,0] + (1-strategy[i-1,0])*payoff_matrix[3,0]
    payoffs[1,1] = strategy[i-1,0]*payoff_matrix[2,1] + (1-strategy[i-1,0])*payoff_matrix[3,1]
